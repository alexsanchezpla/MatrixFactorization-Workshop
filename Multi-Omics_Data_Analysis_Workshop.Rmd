...
-   Learn to apply a variety of single and multiple matrix factorization techniques.
-   Understand the difference between PCA, ICA and NMF through some examples.
-   Understand why we may need to resort to multi-omics analysis and how to do using either MFA or joint matrix factorization
...
-   The supplementary materials from the paper [Enter the Matrix: Factorization uncovers knowledge from omics](https://www.sciencedirect.com/science/article/pii/S0168952518301240). The paper contains very nice examples but unfortunately (oh surprise!) I have been unable to find the data.
...
In this example we work with  al leukemia data set that has been derived from the [`leukemiasEset` Bioconductor package](https://bioconductor.org/packages/release/data/experiment/html/leukemiasEset.html).
...
Alternatively if you succeeded to install the `CompGenomRData`package you can use:
...
One of its most interesting functionality is that sub-setting is applied on all related "sub-objects" (slots).
...
Grouping is done by hierarchical clustering.
...
Our goal is usually to see the samples in the space derived from genes space but of lesser dimension. Notice that for making this possible  we transpose the matrix so samples are on the columns.
...
For this use case we use thee distinct omics that have been downloaded from the TCGA database. 
...
- _Mutation Data_ In the mutation matrix, each cell is a binary 1/0, indicating whether or not a tumour has a non-synonymous mutation in the gene indicated by the column. These types of mutations change the aminoacid sequence, therefore they are likely to change the function of the protein.
...
csvfile <- "datasets/multi-omics/COREAD_CMS13_subtypes.csv"
...
Given that the distinct experiments have distinct scales and distinct variances we scale each dataset by doing:
$$
X_n=\left[
\begin{array}{c}
X_1/\lambda_1^{(1)} \\
...
where $\lambda_1^{(i)}$is the first eigenvector obtained when computing the PCA from $X_i$.
...
\right]
$$
where $X_i^N$ is the normalized data matrix $X_i^N=\frac{x^{ij}}{\sum_j x^{ij}}$ y $\alpha_i=\|X_i^N\|_F$.
...
iCluster+ learns factors which allow tumour sub-types CMS1 and CMS3 to be discriminated.
