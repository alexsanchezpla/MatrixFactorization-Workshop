...
-   Chapters 4 and 11 of the excellent book [Computational Genomics with R](https://compgenomr.github.io/book/)
-   The suplementary materials from the paper [Enter the Matrix: Factorization uncovers knowledge from omics](https://www.sciencedirect.com/science/article/pii/S0168952518301240)

Both sources are freely available on the web.

Although some of the code has been adapted for completeness and ease of use, credit has to be given to the authors of these materials.
...
We start with a subset (1000 genes) of gene expression matrix obtained from the package `leukemiasEset` .

The data have been pre-processed with the code below that keeps the 1000 most variable genes.

This dataset and other used in the workshop are available in package `compGenomDataR`.

Given that this package is rather *heavy to download and install* we have extracted this dataset and the ones for the multi-omics example and have put them in the subdirectory "data". That is, data can be directly read from inside your rstudio project.

Last it has to be noticed that, for this example we would not even need the data because the code below shows how to recreate the dataset.
...
if (!exists("datasets/leukemiaExpressionSubset.rds")){
  ...
  mat1=assay(leukemiaSummExp)
  mat1=(limma::normalizeQuantiles(mat1))
...
<!-- ## Use case II: Revealing distinct data characteristics using distinct factorizations. -->

<!-- This example uses expression data, downloaded from the GTEx database  -->

<!-- - to illustrate how to explore the data using several factorization techniques and  -->

<!-- - to show how distinct factorizations uncover distnct characteristics from the data. -->
